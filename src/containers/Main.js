...
class Main extends Component {
  ...
  searchArticle(e) {
    ...
    axios
      .get(`/articles/title/${e.target.value}`)
      .then((res) => {
        console.log(res.data);
        this.setState({FilterArticles: res.data})
      });
  ...
  render() {
    return (
      <div style={style.container}>
        ...
        <ArticleBlock articleClick={(e, id) => this.articleClick(e, id)}
                      articles={this.state.FilterArticles ? this.state.FilterArticles : this.props.articles}/> {/*假設重新回到此頁面時state會被清空，此時則直接讀取props而飛state*/}
